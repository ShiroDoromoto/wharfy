...
func buildStatus(ctx context.Context, probe bool) (statusOutput, error) {
	...
	// 管理ブロックが無い(init 未実施)か、古い wharfy が書いたまま残っている場合に気づかせる(非致命)。
	// コード付きなので agent は status --json を読んで自力で wharfy init に回れる。
	if warn, next, ok := initAdvice(); ok {
		out.Warnings = append(out.Warnings, warn)
...
func assessChannel(ctx context.Context, ch config.ResolvedChannel, cfg config.Config, in config.File, st *state.State, probe bool, tag string) (statusChannel, *output.Warning) {
	...
	case "winget":
		return assessGatedPR(ctx, cs, st.Publish["winget"], "winget")
	case "homebrew-core":
...
func assessGatedPR(ctx context.Context, cs statusChannel, rec state.PublishRecord, label string) (statusChannel, *output.Warning) {
	...
	if rec.PR != "" {
		...
		if err != nil {
			warn = probeFailedWarning(label+" PR", err)
		} else {
	...
	if cs.State == "pr_open" {
		warn = &output.Warning{Code: output.WarnGatedPending, Message: label + " PR awaiting review: " + rec.PR}
	}
	return cs, warn
}

...
// goreleaser.go — 実効設定から GoReleaser 設定を生成する。
//
...
// glDockerV2 は OCI マルチアーキイメージ。per-arch ビルド＋manifest list を 1 ブロックに
...
// DistDir はビルド成果物の出力先(.wharfy 配下＝利用者 root を汚さない)。
...
func GenerateGoReleaser(cfg Config, in File) ([]byte, error) {
	...
	// release: github(owner/repo)が解決できる時だけ。最終フォールバック列。
	...
	// container: ghcr のマルチアーキ OCI(linux amd64/arm64)。dockers_v2 1 ブロックで出す。
...
func marshalGoReleaser(gl glConfig) ([]byte, error) {
	...
	header := "# GENERATED BY wharfy — do not edit. Source of truth is wharfy.yaml.\n" +
		"# Owned distribution artifact. Lives under .wharfy/.\n"
	return append([]byte(header), body...), nil
}

// WriteGoReleaser は生成設定を <root>/.wharfy/goreleaser.yaml に書く。
// 利用者リポジトリ root には決して書かない(非破壊境界)。書いたパスを返す。
...
// wharfy は .gitignore を**書かない**。next: で追記を提案するための判定だけ行う(冪等)。
